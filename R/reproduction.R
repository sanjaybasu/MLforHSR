...
outprob = (rowMeans(coefs*covars[,influencers])/dim+covars[,round(3*dim/20)]*covars[,round(4*dim/20)]/2-covars[,round(5*dim/20)]*covars[,round(7*dim/20)]/2+covars[,round(3*dim/20)]*covars[,round(4*dim/20)]/2*covars[,round(5*dim/20)]*covars[,round(7*dim/20)]/2+rnorm(pop, mean =0, sd = .01)) 
outprob[outprob>1]=1
outprob[outprob<0]=0
set.seed(1200)
...
perfml <- h2o.ensemble_performance(fit, newdata = test) # assessing cross-validated C-stats
...
library(cvAUC)
perffulllog = predict.glm(fulllogmodel,newdata=testSplit) # apply conventional logistic model with all predictors to test data
perfaiclog = predict.glm(aiclogmodel,newdata=testSplit) # apply conventional logistic model with backwards variable selection by AIC to test data
perffull <- cvAUC(perffulllog, testSplit$y) # cross-validated C-stats for conventional logistic model with all predictors
perfaic <- cvAUC(perfaiclog, testSplit$y) # cross-validated C-stats for conventional logistic model with backwards variable selection by AIC
